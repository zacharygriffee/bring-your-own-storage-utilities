...
export function fromRandomAccess(randomAccessFunction, readdir$) {
    return {
         ...
         * @memberOf fromRandomAccess
         * @param {string} file File name to get
         * @param [config] as well as the configuration below, this is passed to the factory function.
         * @param {string | {encode, decode}} [config.encoding="utf8"]  Either a `string` or an object containing
         * an `encode` and `decode` function.
         * @returns {Promise<*>}
         */
        ...
        (resolve, reject) => {
            ras.stat((e, stat) => {
                ...
                ras.read(0, stat.size, (e, buffer) => {
                    if (e) return reject(e);
                    resolve(encoding.decode(buffer));
                })
            });
        }
    )
