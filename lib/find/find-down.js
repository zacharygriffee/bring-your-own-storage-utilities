...
import {isAbsolute} from "../query/index.js";
import {isRelative} from "../query/index.js";
...
export function findDownMultiple$(source, fileList, config = {}) {
    ...
    return traverseDownDirectory$(source, {...config, cwd: adjustedCwd === "." ? "/" : adjustedCwd})
        .pipe(rx.filter(x => {
            ...
            filter ? rx.filter(filter) : rx.identity,
            rx.take(count),
            rx.takeWhile((x, i) => i <= maxLevel),
            rx.map(x => [x, path.dirname(x).split("/").length - 1]),
            rx.reduce((acc, [x, lvl]) => {
