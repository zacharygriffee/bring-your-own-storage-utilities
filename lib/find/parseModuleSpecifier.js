...
export function parseModuleSpecifier(moduleId, config = {}) {
    ...
    if (_.startsWith(_.trim(moduleId, "/."), "node_modules")) {
        const x1 = _.trim(moduleId, "/.").split("/");
        x1.shift();
        moduleId = x1.join("/")
    }
    ...
    if (type) {
        ...
        if (_.endsWith(moduleId, "/+esm")) {
            moduleId = moduleId.slice(0, -5);
        }
    }

    let p1, p2;
    if (_.startsWith(moduleId, "@")) {
    ...
    const pathName = `${resultPath ? "/" + _.trimStart(resultPath, "/") : ""}`;
