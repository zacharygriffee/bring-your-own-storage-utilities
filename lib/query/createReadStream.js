 ...
 * @param [config.wait=false] a common setting on network storage sources, unless you really want to wait,
 * I have set the default automatically false to only queue locally cached resources.
...
export function createReadStream(source, key, config = {}) {
    ...
    // Covering some side cases of native
    // create read stream.
    if (size) {
        restConfig.size = size;
        restConfig.length = size;
    }

    if (offset) {
        restConfig.offset = offset;
        restConfig.start = offset;
    }
    ...
    if (source.createReadStream) {
        ...
        Promise.resolve(
            ...
            data => {
                if (isString(data)) data = b4a.from(data);
