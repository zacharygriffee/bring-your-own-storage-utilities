...
const rx = {
    catchError, first, map, switchMap, firstValueFrom, of
}
...
export function isFolder(driveSource, path) {
    return rx.firstValueFrom(isFolder$(driveSource, path))
}
...
export function isFolder$(driveSource, path) {
    ...
    return isFile$(driveSource, path).pipe(rx.switchMap(itsAFile => {
        if (!!itsAFile) {
            return rx.of(false)
        }
        ...
        return readdir$(driveSource, {cwd: path}).pipe(rx.first(), rx.map(() => true))
