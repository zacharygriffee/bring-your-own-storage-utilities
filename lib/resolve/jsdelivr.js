...
import {catchError, defaultIfEmpty, filter, map, switchMap, retry, toArray, tap, timeout} from "rxjs/operators";
...
export function getModuleCode$(moduleId, config = {}) {
    ...
    return module ? rx.raceUrls$(module.paths, {method: "get"}) : rx.of(null);
}
...
export function getPackageJson(moduleId, config = {}) {
    return rx.firstValueFrom(
        getPackageJson$(moduleId, config).pipe(
 ...
 * @param {string} moduleId module specifier
 * @param config
 * @memberOf Resolve.JsDelivr
 */
export function exists$(moduleId, config = {}) {
    config.host ||= HOST;
    config.type ||= "npm";
    config.mapPath ||= (path) => path = `${trimEnd(path, "/")}/+esm`;
    ...
    return rx.raceUrls$(module.paths, {method: "HEAD"}).pipe(
        rx.map(headers => {
            // console.log(moduleId, "ModuleId exists", headers.get("Content-Type"))
            return !!headers.get("Content-Type")?.startsWith("application/javascript")
        })
    );
}
