...
import {catchError, defaultIfEmpty, filter, map, switchMap, retry, toArray, tap} from "rxjs/operators";
...
export function exists$(moduleId, config = {}) {
    ...
    return rx.raceUrls$(module.paths, {method: "HEAD"}).pipe(
        rx.map(o => {
            return !!o.headers.get("Content-Type")?.startsWith("application/javascript")
        })
    );
}
