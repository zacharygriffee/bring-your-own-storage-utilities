 ...
 * @param config.path Path of an aspect of the module.
 * @param [config.timeout=60s] Not implemented yet.
 * @returns {{path: string, paths: (*|string[]), module: string, namespace: string, name, type: string, version: string, hash: *}}
 */
export function parseModuleRequest(moduleId, config = {}) {
    config.host ||= HOST;
    config.type ||= "npm";
    return parseModuleSpecifier(...arguments);
}
