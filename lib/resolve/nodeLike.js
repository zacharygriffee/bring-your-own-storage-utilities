import {
    parseModuleSpecifier, resolveExports, // from find/resolvePackageExportsImports.js
    findNodeModule$
} from "../find/index.js";

import {
    loadPackageJson$
} from "./loadPackageJson.js";
...
import {
    EMPTY, defer, firstValueFrom, from, identity, of, concat
} from "rxjs";
import {
    catchError,
    concatAll,
...
import {isArray, isObject, isString, map, reduce, trimStart} from "../util/index.js";

const _ = {isArray, isObject, isString, map, reduce, trimStart, uniqueId};
const rx = {
    EMPTY,
    defer,
    firstValueFrom,
...
function loadFile$(source, id, from = "/", config = {}) {
    const {
        tryExtensions = ["", ".js", ".json", ".mjs", "/package.json"], getter, encoding, ...restConfig
    } = config;
    ...
    if (tryExtensions.includes("")) ids.unshift(id);

    return rx.concat(ids).pipe(rx.concatMap(x => rx.defer(() => getter(source, config.state.id = x, restConfig))), // rx.tap(x => console.log("Got this", x, {source, id, from, config})),
        filterNil(), rx.map(encoding.decode), concatIfEmpty(rx.defer(() => loadDirectory$(source, x, {
            tryExtensions, getter, ...config
        }))), rx.catchError(e => {
            return rx.EMPTY;
        }))
}
...
function loadDirectory$(source, id, config) {
    const {
        exportConditions = ["browser", "import", "es2015", "default"], state
    } = config;
    ...
    return loadPackageJson$(source, {...config, cwd: id}).pipe(filterNil(), // Todo, this is a mess.
        rx.toArray(), rx.map(o => o.reverse()), rx.concatAll(), rx.concatMap(pkg => {
...
function loadIndex$(source, x, config = {}) {
    const {
        tryExtensions, tryIndex = ["/index"], getter, encoding
    } = config;

    // console.log("loadIndex$", x, {source, config});

    if (x.endsWith("index")) return rx.of(null);

    const idsIndexes = _.map(tryIndex, index => _.map(tryExtensions, ext => path.resolve(x + index + ext))).flat();

    return rx.concat(idsIndexes)
        .pipe(rx.concatMap(x => getter(source, config.state.id = x, config)), filterNil(), rx.map(encoding.decode))
}
...
export function nodeLikeResolver$(sources, id, from = "/", config = {}) {
    ...
    if (_.isArray(sources)) {
        return rx.from(sources).pipe(rx.concatMap(source => nodeLikeResolver$(source, id, from, config)))
    }
    ...
    config.state = {
        ...config.state ||= state, entryId: id, entryFrom: from, keyValueMode, debugId: _.uniqueId("resolve$")
    }
    ...
    if (config.state.type === "bare") {
        seqEvents.push(rx.defer(() => findNodeModule$(source, config.state.id = id))
            .pipe(filterNil(), rx.concatMap(x => loadFile$(source, x, undefined, {
                ...config, encoding, getter
            })), rx.defaultIfEmpty(null),))
    } else {
        config.state.type ||= isAbsolute(id) ? "absolute" : "relative"
        seqEvents.push(rx.defer(() => loadFile$(source, config.state.id = id, from, {...config, encoding, getter})));
    }

    return rx.concat(...seqEvents).pipe(detailed ? rx.map(o => ({
        success: !!o, code: o, ...config.state
    })) : rx.identity)
}
...
export async function nodeLikeResolver(sources, id, from, config = {}) {
    return rx.firstValueFrom(nodeLikeResolver$(sources, id, from, config)
        .pipe(rx.take(1), rx.defaultIfEmpty(config.detailed ? {} : null)));
}
