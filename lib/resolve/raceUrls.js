...
import {
    firstValueFrom, from, race, defer, EMPTY
} from "rxjs";
...
const rx = {
    from, defer, filter, map: rxMap, catchError, concatIfEmpty, defaultIfEmpty, concatMap, race, EMPTY
};
...
export function raceUrls$(urlArray, config = {}) {
    ...
    return rx.race(...map(urlArray, attempt => {
        return rx.defer(() => fetch(attempt, {...restOptions, method}))
            .pipe(rx.filter(response => {
                ...
                if (method === "head") {
                    return rx.of(o.headers);
                }
                return rx.from(o.text()).pipe(rx.map(buff => {
                    console.log();
                    o.data = buff;
                    return o
                }));
            }), rx.map(o => {
                if (method === "head") return o;
                ...
                // }
                return o;
            }), rx.concatIfEmpty(() => rx.EMPTY), rx.catchError((e) => {
                return rx.EMPTY
            }));
    }));
}
